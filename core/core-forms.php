...
class AF_Core_Forms {
	...
	function pre_form() {
		...
		if ( isset( $_POST['af_form'] ) && ! isset( $_POST['action'] ) ) {
			...
			if (isset($_POST['full_name_1195']) && !empty($_POST['full_name_1195'])):
				return false;
			endif;
			...
			if ( $form && acf_validate_save_post( true ) ) {
				...
				if ( isset( $_POST['acf'] ) ) {
					...
					foreach ( $_POST['acf'] as $k => $value ) {
						...
						if ($field['type'] != 'repeater' && $field['name'] != 'Files'):
							$fields[] = $field;
						endif;

					}
					
					if ($GLOBALS['solten_form_files']):
						$fields['solten_files'] = $GLOBALS['solten_form_files'];
						$GLOBALS['solten_form_files'] = array();
					endif;
				}
